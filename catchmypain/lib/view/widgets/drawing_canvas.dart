...
class DrawingCanvas extends ConsumerWidget {
  ...
  DrawingCanvas({super.key, required this.height, required this.width,});
    ...
    Widget buildCurrentPath(BuildContext context, ref) {
      ...
      return Listener(
        onPointerDown: (details) {
          ...
          ref.read(sketchProvider.notifier).state = Sketch(points: [offset], color: selectedColor, size: strokeSize, mode: drawingMode);
        },    
        onPointerMove: (details) {
          ...
          ref.read(sketchProvider.notifier).state = Sketch(points: points, color: selectedColor, size: strokeSize, mode: drawingMode);
          }
        ,
        onPointerUp: (details) {
          ref.read(sketchesProvider.notifier).state = List<Sketch>.from(allSketches) ..add(currentSketch!);  
          // 현재 스케치를 초기화합니다.
           ref.read(sketchProvider.notifier).state = Sketch(points: [], color: selectedColor, size: strokeSize, mode: drawingMode);       
          },
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (Sketch sketch in sketches) {
      ...
      for (int i = 1; i < points.length - 1; i++) {
        final p0 = points[i];
        final p1 = points[i+1];
        path.quadraticBezierTo(p0.dx, p0.dy, (p0.dx+ p1.dx) / 2, (p0.dy + p1.dy) / 2);    // 이기 뭐꼬
      }

      Paint paint = Paint() ..color = sketch.color ..strokeWidth = sketch.size ..strokeCap = StrokeCap.round ..style = PaintingStyle.stroke;
